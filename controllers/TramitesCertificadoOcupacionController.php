...
class TramitesCertificadoOcupacionController extends Controller
{
    ...
    public function actionSalvar() { 
        ...
        if($pasoIndex==2){
	        $error=$this->salvarArchivos($transaction,$model,$encabezado,'p2PlanoAutorizado','Plano Autorizado');
          if($error!="OK")
              return $this->cancelarSalvar($transaction,$error);
        }
        if($pasoIndex==2){
	        $error=$this->salvarArchivos($transaction,$model,$encabezado,'p2Licencia','Licencia');
          if($error!="OK")
              return $this->cancelarSalvar($transaction,$error);
        }
        if($pasoIndex==2){
        ...
        if($pasoIndex==2){
	        $error=$this->salvarArchivos($transaction,$model,$encabezado,'p2Vitacora','Bitacora');
          if($error!="OK")
              return $this->cancelarSalvar($transaction,$error);
        }
        if($pasoIndex==4){
        ...
        if($pasoIndex==6){
	        $error=$this->salvarArchivos($transaction,$model,$encabezado,'p6Certificado','Certificado');
          if($error!="OK")
              return $this->cancelarSalvar($transaction,$error);
        }
