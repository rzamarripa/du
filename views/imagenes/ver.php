...
<div id="imagen">
  ...
  <div class="col-sm-8 col-sm-offset-2">  
    ...
    <div class="col-sm-12">
      <?php if($paginacion != 1){?>
      <button class="btn btn-primary" onclick="cambiar(<?= $consecutivo-1; ?>,<?= $paginacion-1; ?>)"><li class="fa fa-arrow-left"></li></button>
    <?php } ?>
    <?= $paginacion.'/'.$totalImagenes ?>
    <?php if($paginacion != $totalImagenes){?>
      <button class="btn btn-primary" onclick="cambiar(<?= $consecutivo+1; ?>,<?= $paginacion+1; ?>)"><li class="fa fa-arrow-right"></li></button>
    <?php } ?>
...
<script type="text/javascript">
  function cambiar($c,$paginacion) {
    $.ajax({
      type: 'POST',
       url: 'ver',
       data: {consecutivo: $c, paginacion: $paginacion, id: <?php if(isset($_GET['id'])){ echo $_GET['id'];}else{echo $_POST['id'];} ?>,
